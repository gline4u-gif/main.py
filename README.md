...
def is_market_open():
    ...
    # Check if it is the weekend (Monday=0, Sunday=6)
    if now.dayofweek > 4:
        return False
...
def get_intraday(session, symbol):
    ...
    # CHANGED: Interval is now 5 minutes
...
def main():
    print("Starting 5-min intraday data fetch pipeline...")
    ...
    print("\n✅ All 5-Min Intraday Data Uploaded Successfully")

if __name__ == "__main__":
    while True:
        if is_market_open():
            print("Market is open. Running 5-min data fetch...")
            main()
            print("Fetch complete. Waiting 5 minutes before the next run...")
            # CHANGED: Sleep for 300 seconds (5 minutes) instead of 900
            time.sleep(300)  
        else:
            print("Market is currently closed. Sleeping for 1 hour...")
            time.sleep(3600) # Sleep for 1 hour to save resources
